...
export async function POST(req: Request) {
  try {
    ...
    const rateError = checkRateLimit(authResult.userId, "agents-run");
    ...
    if (agentIds.length > MAX_AGENTS_PER_RUN) {
      return NextResponse.json({ error: `Maximum ${MAX_AGENTS_PER_RUN} agents per run.` }, { status: 400 });
    }
    ...
    if (mode === "parallel") {
      ...
      for (const s of settled) {
        if (s.status === "fulfilled") {
          ...
          results.push({ agentId: "unknown", name: "Unknown", error: "Unexpected failure.", ok: false });
        }
      }
    } else {
      let contextSoFar = "";
