...
export async function POST(req: Request) {
    ...
    try {
        ...
        estimatedCostCents = estimateDirectorCost(String(body?.model || ""));

        const debit = await deductApiBalanceCents({
            userId,
            amountCents: estimatedCostCents,
            reason: "api_v1_director_generate",
            apiKeyId,
        ...
        if (balanceDebited && userId) {
            await addApiBalanceCents({
                userId,
                amountCents: estimatedCostCents,
                reason: "api_v1_director_refund_exception",
