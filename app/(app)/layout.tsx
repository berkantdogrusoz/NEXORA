...
const menuItems = [
    ...
    { href: "/assistant", label: "Library", icon: Library },
    { href: "/generate", label: "Image", icon: Image },
    { href: "/studio", label: "Video", icon: Clapperboard },
    { href: "/director", label: "Blueprints", icon: WandSparkles },
];
...
function AppLayout({
    ...
    return (
        <div className="flex min-h-screen bg-black text-white font-sans">
            ...
            {/* ═══════════════════════════════════════════ */}
            <aside
                className={`fixed inset-y-0 left-0 z-50 w-[240px] md:w-[92px] bg-[#070809] border-r border-white/[0.08] flex flex-col transform transition-transform duration-300 ease-in-out md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
            >
                {/* Logo */}
                <div className="px-3 py-4 border-b border-white/[0.08]">
                    <Link href="/" className="flex items-center md:justify-center gap-3 group">
                        <div className="relative w-10 h-10 flex items-center justify-center rounded-2xl bg-gradient-to-br from-white/20 to-white/5 border border-white/15">
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                href={item.href}
                                onClick={() => setSidebarOpen(false)}
                                className={`group flex items-center md:flex-col md:justify-center gap-2 md:gap-1 px-3 py-2.5 rounded-2xl text-[13px] font-semibold transition-all duration-200 relative overflow-hidden border ${isActive
                                    ? "bg-white/10 text-white border-white/20"
                                    : "text-white/55 hover:text-white hover:bg-white/[0.04] border-transparent"
                                    }`}
                            >
                                <Icon className={`w-4 h-4 md:w-5 md:h-5 flex-shrink-0 ${isActive ? "text-white" : "text-white/40 group-hover:text-white/80"} transition-colors`} />
                                <span className="uppercase tracking-wider md:text-[10px] md:leading-none">{item.label}</span>
                            </Link>
                        );
                    })}
                </nav>
                    ...
                    >
                        <div className="flex items-center justify-between md:flex-col md:gap-1">
                            <div className="flex items-center gap-2 md:gap-1">
                                ...
                                <span className="text-[10px] font-bold text-white/55 uppercase tracking-[0.2em] md:tracking-normal">
                                    CR
                                </span>
                            </div>
            ...
            {/*   MAIN CONTENT AREA                         */}
            {/* ═══════════════════════════════════════════ */}
            <div className="flex-1 md:ml-[92px] relative min-h-screen">
                {/* Mobile header */}
